...
func Test_AddContext_AddsNewContext(t *testing.T) {
	rc := &RuntimeConfiguration{}

	result := rc.AddContext(&Context{Name: "a"})
...
func Test_AddContext_ReturnsErrorIfContextExists(t *testing.T) {
	rc := &RuntimeConfiguration{
		contextMap: toContextMap([]Context{
	...
	result := rc.AddContext(&Context{Name: "a", ServerURL: "modified"})

	assert.Equal(t, "a context with the name \"a\" already exists", result.Error())
}

func Test_AddContext_SetsCurrentContext_WhenNotSet(t *testing.T) {
	rc := &RuntimeConfiguration{}
